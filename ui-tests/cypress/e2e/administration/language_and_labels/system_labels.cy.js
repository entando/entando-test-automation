...
describe('Labels', () => {
  ...
  describe('Update labels list functionalities', () => {
    ...
    it([Tag.SANITY, 'ENG-3238', 'ENG-3971'], 'When deletion is confirmed, modal should close and list should be updated', () => {
      addRandomLabel().then(label =>
          openLabelsPage()
              .then(page => page.getContent().getKebabMenu(label.key).open().clickDelete())
    ...
    it([Tag.SANITY, 'ENG-3238', 'ENG-3971'], 'When deletion is canceled, modal should close and the label should still be present', () => {
      addRandomLabel().then(label =>
          openLabelsPage()
              .then(page => page.getContent().getKebabMenu(label.key).open().clickDelete())
